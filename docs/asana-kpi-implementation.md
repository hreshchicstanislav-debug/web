...
**Важно:** Если задача завершена (`completed = true`), но поле "когда обработал" не заполнено (`processed_at IS NULL`), то используется `completed_at` как дата обработки. Таким образом, завершённая задача всегда попадает в "Сделано", даже если дата обработки не проставлена вручную.
...
**Описание:** Сумма `q` задач, где товар уже принесли фотографу, но работа ещё не начата (товар физически находится у фотографа).
...
**Важно:** Если товар сфотографирован (`shot_at` заполнено), он считается возвращённым и исчезает из "Уже на руках", переходя в "Сфоткано, но не обработано".

**Где считается:**
- Файл: `/Users/stanislav/supabase/functions/fetch-asana-stats/index.ts`
- Функция: `computeWeekAggregates()` → цикл по `planTasks` (строка 473-499)
- Условие: `if (!task.completed && task.product_source === PRODUCT_SOURCE_PRINESLI && !task.shot_at)` (строка 487)
