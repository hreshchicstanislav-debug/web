...
Для запуска локального dev-сервера используйте один из способов:

**Способ 1: Использование скрипта для Timetrack (рекомендуется)**
```bash
./start-server-timetrack.sh
```
Сервер запустится на порту **4173** (http://localhost:4173)

**Способ 2: Использование основного скрипта с указанием порта**
```bash
./start-server.sh 4173
```
или через переменную окружения:
```bash
PORT=4173 ./start-server.sh
```

**Способ 3: Использование основного скрипта (порт по умолчанию)**
```bash
./start-server.sh
```
Сервер запустится на порту **8000** (http://localhost:8000)

### Важно: Конфликт портов

Если вы работаете одновременно с проектом **galleries-app**, убедитесь, что используются **разные порты**:

- **Timetrack**: используйте порт **4173** (через `./start-server-timetrack.sh`)
- **galleries-app**: обычно использует порт **3000** или **8000**

Это позволит избежать конфликтов при параллельной разработке.
...
1. Запустите локальный сервер (см. раздел "Локальный запуск" выше)
2. Откройте в браузере адрес сервера (например, http://localhost:4173)
3. Введите план рабочего дня (начало и конец)
4. Введите факт прихода и ухода
5. Добавляйте отлучки при необходимости
6. Система автоматически рассчитает баланс дня и общий итог с учетом предыдущих дней
