...
DECLARE
  week_start_val DATE;
  week_end_val DATE;
  prev_week_start_val DATE;
  done_fact INTEGER;
  to_shoot INTEGER;
  week_load_val INTEGER;
  plan_val INTEGER;
  carry_over_val INTEGER;
  on_hand INTEGER;
  warehouse INTEGER;
...
BEGIN
  ...
  week_start_val := DATE_TRUNC('week', target_date)::date + INTERVAL '1 day' * (1 - EXTRACT(DOW FROM DATE_TRUNC('week', target_date)::date));
  week_end_val := week_start_val + INTERVAL '6 days';
  
  -- Вычисляем начало предыдущей недели
  prev_week_start_val := week_start_val - INTERVAL '7 days';
  ...
  WITH fact_tasks AS (
    ...
    WHERE t.q > 0 
      ...
      AND (
        t.week_processed = week_start_val 
        OR t.week_shot = week_start_val
        OR (t.completed = true AND t.completed_at IS NOT NULL AND 
            DATE_TRUNC('week', t.completed_at::date)::date + INTERVAL '1 day' * (1 - EXTRACT(DOW FROM DATE_TRUNC('week', t.completed_at::date)::date)) = week_start_val)
      )
  )
  SELECT 
    COALESCE(SUM(q), 0),
  ...
  -- Используем явное имя таблицы, чтобы избежать конфликта с переменными
  ...
  overtime_qty := GREATEST(0, (done_fact + carry_over_val) - plan_val);
  
  -- Выполняем UPSERT в asana_stats
  -- Используем имя индекса для избежания конфликта с переменными
  ...
  VALUES (
    ...
    week_load_val,
    plan_val,
    done_fact + carry_over_val, -- done_qty
    carry_over_val,
    on_hand,
    warehouse,
    shot_not_processed,
